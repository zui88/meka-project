...
\section{Beschaffen der Trainingsdaten} \label{sec:Frame1Label}
Zum Ausführen des Livescripts werden aufbereitete Trainingsdaten benötigt. Deren Beschaffung und Aufbereitung wird hier beschrieben.

\subsection{Bilder}
Zu Beginn des Projektes wurden Beispielbilder von Matlab als Datenbasis verwendet. Dabei stellt Matlab Datensätze mit bestimmten Objekt-Klassen bereit. Es sind Bildersätze von Autos oder Fahrräder oder Fußgängern. Ein Datensatz beinhaltet Bilder mit den beiden Klassen 'Fahrrad' und 'Person'. Dabei ist in jedem Bild jeweils ein klar definiertes, offensichtliches Objekt.\\
Im Straßenverkehr herrscht aber eine andere Situation. Dort gibt es Autos in unterschiedlichen Entfernungen. Manche Autos sind nur teilweise zu sehen. Es ist also viel schwerer abzugrenzen, was als Objekt erkannt werden soll und was nicht. Des Weiteren werden natürlich viele verschieden Objekt-Klassen vorkommen. Das Verhältnis der Anzahl, der Objektarten zueinander wird auch stark schwanken. Deshalb haben wir während Autofahrten in städtischen Bereich Bilder und Videos (siehe Kapitel \ref{Kap:VideoLabel}) aufgenommen. Um diese las Datensatz für das Training verwenden zu können.


\subsection{Bilder-Label}
Da ''yolov2'' zur Kategorie des supervised Learnings gehört, müssen die Bilder gelabeled werden. Die oben beschriebenen  Matlab-Bilder wurden entsprechend gelabeled. Dabei wurde nicht immer der ganze Datensatz sondern, einfach einige Bilder gelabeled um sie später als Trainings doer Testdatensatz verwendet werden zu können.\\
Beim Labeln der selbst aufgenommenen Bilder wurden folgende Labelsätze erzeugt:
\begin{itemize}
	\item \textbf{'labeledPictures0014.mat'}: Label für die ersten 200 Bilder. Dabei sind in einem Bild maximal 4 Objekte gelabeled.
	\item \textbf{'labeledPictures0024.mat'}: Label für alle 400 Bilder. Dabei sind in einem Bild maximal 4 Objekte gelabeled.
	\item \textbf{'labeledPictures0036.mat'}: Label für alle 400 Bilder. Dabei sind maximal 6 Objekte in einem Bild gelabeled.
	\item \textbf{'labeledPictures00410.mat'}: Label für alle 400 Bilder. Dabei sind maximal 6 Objekte in einem Bild gelabeled.
\end{itemize}

Die zugehörige Label-Sessions sind entsprechend im Ordner ''LabelSessions'' abgespeichert und als ''imageLabelingSessionXXXXpictures.mat'' benannt. \\
\textbf{Hinweis: }Die maximale Menge an Label pro Bild ist  durch die Anordnung der Layer des KNN limitiert beziehungsweise vorgegeben (siehe Abschnitt \ref{sec:Frame1Lgraph}).Da aber oft sehr viel Objekte in einem Bild zu finden sind wurden die Datensätze wie oben beschrieben erzeugt.\\

Jede dieser Dateien besteht aus einem Objekt der Matlab-Klasse 'gTruth'. Dieses Objekt wird vor dem Training in den Workspace von Matlab geladen. 'gTruth' besteht aus:
\begin{itemize}
	\item \textbf{'DataSource'}:Beinhaltet den Pfad und Dateinamen jedes gelabelten Bildes.
	\item \textbf{'LabelDefinitions'}:Hier sind die Definitionen der Unterschiedlichen Objektklassen festgehalten.
	\item \textbf{'LabelData'}: Beinhaltet die eigentlichen Label. Diese liegen in folgender Form vor: [x,y,b,h]. Dabei beschreiben sind x und y die Koordinaten der linken, oberen Ecke des Bildes. 'b' stellt die Breite des Labels in Pixeln (in x-Richtung) und h die Höhe des Labels (in y-Richtung) dar.
\end{itemize}

\subsection{Image-Labeler} \label{sec:Frame1ImageLabeler}
Matlab stellt zum Erzeugen der Label eine App, den sogenannten 'ImageLabeler' zur Verfügung.

\subsubsection{Erzeugen einer Label-Session}
 Durch den Befehl 'ImageLabeler' im Command-Window öffnet sich diese App. \\
Durch Drücken des ICON ''New Session'' und dem Laden von Bildern über ''Load/Add Images from Folder'', lässt sich eine Labelsession erstellen.\\
Anschließend müssen die Label-Klassen erzeugt werden (siehe \ref{fig:LabelSessionLabelklasse}). Anschließend werden die Objekte durch Anwählen mit dem entsprechenden Label händisch markiert (siehe \ref{fig:LabelSessionLabel}).\\
Die Session kann während des Erstell-Vorganges unter 'save/Session' gespeichert werden. Hierbei sollte sie in der Matlab-Ordnerstruktur (siehe \ref{sec:Frame1}) im Unterordner 'LabelSessions' gepeichert werden. 

\begin{figure}
	\begin{center}
		\includegraphics[width=0.8 \textwidth]{./figures/LabelSessionLabelklasse.jpg}
		\caption{Erstellung einer Label-Klasse im ImageLabeler}
		\label{fig:LabelSessionLabelklasse}
	\end{center}
\end{figure}

\begin{figure}
	\begin{center}
		\includegraphics[width=0.8 \textwidth]{./figures/LabelSessionLabel.jpg}
		\caption{Erstellung von Labeln im ImageLabeler}
		\label{fig:LabelSessionLabel}
	\end{center}
\end{figure}

\subsubsection{Exportieren der Label}
Über 'ExportLabels/To File' können die Label exportiert und ein Label-Datensatz erzeugt werden. Mit diesem kann später (über Pfade) auf die selben Bilder, wie in der Labelsession verwendet , zugegriffen werden. 


\subsubsection{Bearbeiten bereits erstellter Label und Label-Sessions}
Über 'Load/Session' lässt sich eine Labelsession laden. Dabei sollte der Matlab Pfad auf den Ordner der Labelsession zeigen und der Pfad der Bilder über 'addpath()' zuvor hinzugefügt werden. Wurde die Labelsession jedoch auf einem anderen PC oder in einem Ordner erzeugt, so kann er trotzdem Bilder und die '.mat' Datei der Labelsession nicht einander zuordnen. Ist dies der Fall, so müssen die Pfade im Labelsatz geändert werden. Dazu müssen die Sections 1a),1b) und 1c) aus dem Framework ausgeführt werden. (siehe \ref{sec:Frame1}). Hierbei darf bei der Konfiguration ''useScaledPicture'' NICHT angewählt werden. Da sonst nicht die Originalbilder als Referenz verwendet werden würden.\\
Ist die Labelsession geöffnet, kann sie wie gewöhnliche Dokumente auch geändert und gespeichert und die Label neu exportiert werden.



\section{Livescript-Beschreibung} \label{sec:Frame1}

Mit diesem Livescript kann mithilfe bereits gelabelter Bilder ein Neuronales Netzwerk erstellt, trainiert und evaluiert werden. Dazu sind abhängig von den jeweiligen Anforderungen verschiedene Schritte notwendig. Das Livescript soll dabei interaktiv nutzbar sein und möglichst alle Aufgabenstellungen, die bei vorliegendem Projekt in diesem Teil auftauchen können, abdecken. Das Livescript kann so genutzt werden, dass bestimmte Aktionen, je nach Bedarf entsprechend ausgeführt werden können oder eben auch nicht. Es geht also nicht darum immer das ganze Skript auszuführen.  An verschiedenen Stellen können Aktionen auf verschiedene Art und Weise oder mit wählbaren Parametern oder anderen Datensätze durchgeführt werden.\\
...
Um späteres Zugreifen auf die Ordner der Struktur zu gewährleisten wird die Funktion ''fileparts'' ausgeführt. Diese liefert folgenden Output: 	
\begin{matlabcode}
...
In diesem Teil werden die Datensätze konfiguriert. Dazu stehen dem Anwender sechs Auswahlmöglichkeiten in Form von Fragen zur Verfügung. Diese kann dieser entweder über eine Checkbox mit ''true'' oder ''false'' beantworten, oder über eine Dropdown-Liste eine Antwort auswählen.  	
	\begin{enumerate}
		\item{ Werden skalierte Bilder oder die Originalbilder verwendet? }  						(Checkbox)
		\item{ Sollen bereits skalierte Bilder verwendet werden oder müssen Bilder neu-skaliert werden? }  (Checkbox)
...
\subsubsection{d) Skalieren der Bilder mit den zugehörigen Bounding Boxes:}
...
Für das spätere Abgleichen und Testen des Netzes mit Kameras an bekannten Bildern, die auf dem Bildschirm angezeigt werden, ist es jedoch vorteilhaft, hochauflösende und damit realitätsnähere Bilder zu benutzen. Daher sind hochauflösende Bilder insbesondere für das Testen und Abgleichen und die skalierten Bilder für das Trainieren geeignet.\\
...
Hier werden die Trainingsdaten in einen anderen Matlab-spezifischen Datentyp, dem sogenannten ''Datastore'', welcher später der Trainingsfunktion übergeben wird, konvertiert.
...
Das Seitenverhältnis ('aspectRatio') und die Fläche('area') der Bounding-Boxes werden in einem Scatter-Plot dargestellt(siehe Abbildung \ref{fig:Frame1BoundingBoxes}). Mit Hilfe dieses Plots, kann sich der Anwender einen Überblick über die Größe und Form der Label und deren Streuung beschaffen.
...
\begin{itemize}
	...
	\item \textbf{'Verbose'}: Es handelt sich um eine binäre Variable. Ist diese auf 'true' gesetzt, so werden die Zwischenergebnisse des Trainingsvorganges, wie in Tabelle \ref{tab:Frame1Tainingsvorgang} dargestellt, im Live-Script ausgegeben.(siehe Abschnitt \ref{sec:Frame1Training})
...
Für das Training wird die Matlab-Funktion ''trainYOLOv2ObjectDetector'' eingesetzt. Sie gibt sowohl den Detektor als auch Informationen zum Trainingsvorgang zurück. Der Detektor beinhaltet das trainierte (angepasste) KNN. Warum der Name Detektor angebracht ist, wird in  Abschnitt \ref{sec:Frame1Test} erklärt.\\
Um das Training durchführen zu können, werden folgende Informationen benötigt:
\begin{itemize}
	...
	\item KNN:
	\begin{itemize}
	\item Ein Netzwerk vom Matlabtyp ''lgraph'' aus Abschnitt \ref{sec:Frame1Lgraph}
	\item Ein Detektor aus voran gegangenen Trainingsvorgängen
	\end{itemize}
\end{itemize}
...
Während des Trainingsvorgangs wird die Tabelle \ref{tab:Frame1Tainingsvorgang} in die Ausgabe des Livescripts geplottet. Anhand dieser Tabelle lässt sich der Trainingsvorgang überwachen. So kann beispielsweise die voraussichtliche Dauer des Trainingsvorgangs abgeschätzt werden.
\begin{figure}
	\begin{tiny}
		\begin{matlaboutput}
		*************************************************************************
...
\label{code:Frame1EvalDetectPrec}
\end{figure}

In Abbildung \ref{fig:Frame1EvalDetectPrec} sind die Parameter aus der zweiten Zeile des Matlab-Codes \ref{code:Frame1EvalDetectPrec} geplottet. Die durchschnittliche Präzision der Objekterkennung beträgt 0,95. Der Wert 1,0 würde ein optimale Präzision von 100 Prozent bedeuten. \\
...
Nun wird der Recall angehoben. Es muss also ein höherer Prozentsatz der Objekte detektiert werden. Dadurch wird die Präzision abnehmen, da eben auch einige Objekte fälschlicherweiße erkannt werden. An einem gewissen Punkt wird der Recall so groß, dass alle vom Detektor gefundenen Objekte genutzt werden. Hier bricht der Graph ab. Ein Recall von 1 wird also nicht erreicht. Je weiter rechts beziehungsweise näher am wert 1 dieser Punkt ist, umso besser. \\

Die Abbildung \ref{fig:Frame1EvalDetectPrecThres} zeigt den selben Plot für den selben Detektor und das selbe Netzwerk. Hierbei werden jedoch die Instanzen aus der dritten Zeile des Matlab-Codes \ref{code:Frame1EvalDetectPrec} genutzt . Der Unterschied dabei ist, dass hier die Variable 'threshold' übergeben wird. Der Threshold bezeichnet hier, wie hoch der 'score' der Objekterkennung mindestens sein muss um als detektiertes Objekt zu gelten. Dieser ist default-mäßig auf 0.5 gestellt. Oft ist es jedoch wichtig die Objekte mit einer höheren Sicherheit zu erkennen. Und wie man in den aufgeführten Plots sieht, ist dies deutlich schwerer. Deshalb lässt sich  im Livescript dieser Threshhold zwischen 0 und 1 einstellen um die Auswirkungen auf die Detektorperformance zu untersuchen.\\ 
